    ...
    {screen === 'form' ? <>
      <div className="feedback-d3-scroll">
        {!type && <section className="feedback-d3-intro">
          ...
          <div><Heading variant="hero">直接说，我们在看</Heading><p>哪里难用、客服哪里不爽、想要什么新功能</p></div>
        </section>}

        <section className="feedback-d3-section">
          <Heading variant="section">选择类型 <em>*</em></Heading>
          {!type ? <div className="feedback-d3-type-cards">
            {feedbackTypes.map(({ value, label, detail, Icon }) => <OptionTile key={value} selected={type === value} onClick={() => setType(value)} icon={<Icon size={20} />} title={label} description={detail} />)}
          </div> : <div className="feedback-d3-chips">
            {feedbackTypes.map(({ value, label }) => <ChoiceChip key={value} selected={type === value} showCheck onClick={() => setType(value)}>{label}</ChoiceChip>)}
          </div>}
        </section>

        {type === 'product' && <section className="feedback-d3-section">
          <Heading variant="section">建议方向 <small>（可多选）</small></Heading>
          <div className="feedback-d3-direction-chips">{suggestionDirections.map((value) => <ChoiceChip selected={directions.includes(value)} showCheck key={value} onClick={() => toggleDirection(value)}>{value}</ChoiceChip>)}</div>
        </section>}

        <section className="feedback-d3-section">
          <Heading variant="section">想说什么 <em>*</em></Heading>
          <TextAreaField className="feedback-d3-textarea" maxLength={500} showCount value={content} onChange={(event) => setContent(event.target.value)} placeholder="说得越具体越好：在哪个页面、想做什么、遇到了什么" />
        </section>

        <section className="feedback-d3-section">
          <Heading variant="section">上传截图 <small>（选填，最多 3 张）</small></Heading>
        ...
        <section className="feedback-d3-contact">
          <Heading variant="section">联系方式 <small>（选填）</small></Heading>
      ...
      <footer className="feedback-d3-submit">
        <Button disabled={!canSubmit} onClick={submit}>提交反馈</Button>
...
function FeedbackHistory({ submitted, onReset, onHome }: { submitted: boolean; onReset: () => void; onHome: () => void }) {
  return <div className="feedback-d3-history-scroll">
    {submitted && <section className="feedback-d3-success">
      ...
      <Heading variant="result">已收到，谢谢你说出来</Heading>
    ...
    <section className="feedback-d3-history-list">
      <header><Heading variant="section">我提过的</Heading><span>{historyItems.length}条</span></header>
