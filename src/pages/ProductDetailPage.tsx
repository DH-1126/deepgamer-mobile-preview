  ...
  return <main className="product-detail-page">
    ...
    <div className="product-detail-scroll" ref={scrollRef} onScroll={onScroll}>
      <section className="detail-overview">
        ...
        <div className="detail-game"><img src={detail.gameIcon} alt="" /><b>{detail.gameName}</b><i /> <span>{detail.platform}</span><button className="detail-product-code" type="button" onClick={copyProductCode} aria-label={`复制商品编号 ${detail.productCode}`}>编号 {detail.productCode}<Copy size={12} strokeWidth={2} aria-hidden="true" /></button></div>
      ...
      <section id="detail-panel-guarantee" role="tabpanel" aria-labelledby="detail-tab-guarantee" className="detail-section" ref={(node) => { sectionRefs.current.guarantee = node }}><header className="detail-content-heading"><h2>平台保障范围</h2></header><div className="detail-guarantee-grid"><div><b>✓ 覆盖</b>{detail.guaranteeCovered.map((item) => <span key={item}>{item}</span>)}</div><div><b>× 不覆盖</b>{detail.guaranteeExcluded.map((item) => <span key={item}>{item}</span>)}</div></div><ol className="detail-process">{process.map((item) => <li key={item}>{item}</li>)}</ol><div className="detail-community"><img src={asset('community-logo.png')} alt="" /><span><b>{detail.groupName}</b><small>群号：{detail.groupNumber}</small></span><button type="button" onClick={copyGroup}>复制群号</button></div><div className="detail-tips"><h3>温馨提示</h3>{detail.tips.map((item, index) => <p key={item}>{index + 1}. {item}</p>)}</div></section>
    ...
    <footer className="detail-actions"><button type="button" className={favorite ? 'favorite' : ''} aria-pressed={favorite} onClick={toggleFavorite}><Heart size={21} strokeWidth={2} fill={favorite ? 'currentColor' : 'none'} aria-hidden="true" />{favorite ? '已收藏' : '收藏'}</button><button type="button" onClick={openSupport}>客服</button><button type="button" className="primary" disabled={!canPurchase(detail)} onClick={openPurchase}>立即购买</button></footer>
