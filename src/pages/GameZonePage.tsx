...
import { getPricePreset, getServerOption, getToolbarActiveState, hasInvalidPriceRange, pricePresets, serverPlatforms, type ServerOption, type ToolbarSelection } from '../components/quickFilterModel'
...
export function GameZonePage() {
  ...
  const [quickPanel, setQuickPanel] = useState<'server' | 'price' | null>(null)
  const [toolbarSelection, setToolbarSelection] = useState<ToolbarSelection>('default')
  ...
  const toggleQuickPanel = (panel: 'server' | 'price') => {
    setDrawerOpen(false)
    ...
    else setPriceDraft({ min: filters.minPrice, max: filters.maxPrice, preset: getPricePreset(filters.minPrice, filters.maxPrice) })
    setQuickPanel(panel)
  }
  ...
  return (
    <main className="catalog-page">
      ...
      <section className="catalog-sort" role="toolbar" aria-label="商品排序和筛选">
        <button type="button" className={toolbarActive.default ? 'active' : ''} aria-pressed={toolbarActive.default} onClick={() => { setSort('default'); setToolbarSelection('default'); setQuickPanel(null) }}>综合</button>
