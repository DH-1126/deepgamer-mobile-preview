...
export function FulfillmentContractPage() {
  ...
  if (!contract) return <main className="fulfillment-contract-page"><div className="contract-state"><Heading as="h1" variant="result">合同不存在</Heading><Link to="/message">返回消息</Link></div></main>
  ...
  return <main className="fulfillment-contract-page">
    ...
    <form onSubmit={submit}>
      <div className="contract-scroll">
        ...
        <Heading className="contract-section-label" as="h2" variant="group">合同正文</Heading>
        ...
        <Heading className="contract-section-label" as="h2" variant="group">签署信息</Heading>
        <section className="contract-signer"><div><span>签署人</span><b>{contract.signerMasked}</b></div><div><span>证件号</span><b>{contract.identityMasked}</b></div><TextField className="contract-signature" label="手写签名" hint="本地演示签名，不产生法律效力" value={signature} disabled={signed} onChange={(event) => setSignature(event.target.value)} placeholder="输入本人姓名作为演示签名" aria-label="演示签名" /></section>
        <Checkbox className="contract-agree" checked={agreed} disabled={signed} onCheckedChange={setAgreed} label={<>我已阅读并同意 <b>《游戏账号回收协议》</b> 全部条款</>} />
      </div>
      <ActionBar className="contract-actions" layout="primary-end"><Button type="button" variant="outline" onClick={() => navigate(-1)}>{signed ? '返回群聊' : '稍后签'}</Button><Button type="submit" disabled={signed || submitting || Boolean(submitIssue)} loading={submitting}>{signed ? '已签署' : '签署并提交'}<small>{!signed && submitIssue ? submitIssue : signed ? '本地演示状态已保存' : '演示提交，不具法律效力'}</small></Button></ActionBar>
    </form>
  </main>
}
