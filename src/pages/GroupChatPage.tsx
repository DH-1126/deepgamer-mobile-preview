...
import { ArrowLeft, Camera, RotateCcw, Send } from 'lucide-react'
...
export function GroupChatPage() {
  ...
  const allMessages = useMemo(() => [...messages, ...pending].sort((a, b) => a.createdAt - b.createdAt), [messages, pending])
  ...
  return <main className="group-chat-page">
    <header className="chat-header"><div className="chat-status" aria-hidden="true"><time>9:41</time><span><img src={assetPath('assets/home-v2/status-signal.svg')} alt="" /><img src={assetPath('assets/home-v2/status-wifi.svg')} alt="" /><img src={assetPath('assets/home-v2/status-battery.svg')} alt="" /></span></div><div className="chat-titlebar"><button type="button" onClick={back} aria-label="返回消息"><ArrowLeft size={21} /></button><span><h1>{conversation.kind === 'trade_group' ? `交易群 · ${conversation.title}` : conversation.title}</h1><small>{conversation.kind === 'trade_group' ? '买家 · 卖家 · 平台客服萌萌' : '平台客服'}</small></span>{conversation.orderId ? <Link to={`/orders/${conversation.orderId}`}>订单</Link> : <span />}</div></header>
    ...
    <footer className="chat-composer">
      {!conversation.closed && <div className="chat-quick-actions"><button type="button" disabled={cooldown > 0} onClick={() => quickSend('客服')}>催客服{cooldown > 0 ? ` ${cooldown}s` : ''}</button><button type="button" disabled={cooldown > 0} onClick={() => quickSend('卖家')}>催卖家</button><button type="button" disabled={cooldown > 0} onClick={() => quickSend('买家')}>催买家</button></div>}
