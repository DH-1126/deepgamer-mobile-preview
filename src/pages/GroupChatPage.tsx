...
import { BottomSheet, Heading, PageHeader, StatusBar, ToggleSwitch } from '../components/ui'
...
import { TradeChatPage } from './TradeChatPage'
...
export function GroupChatPage() {
  ...
  if (error) return <main className="group-chat-page"><div className="chat-state" role="alert"><Heading as="h1" variant="result">会话加载失败</Heading><p>{error}</p><button type="button" onClick={() => { setLoading(true); void load() }}>重试</button></div></main>
  if (!conversation) return <main className="group-chat-page"><div className="chat-state"><Heading as="h1" variant="result">会话不存在</Heading><p>该交易群可能已关闭或编号有误。</p><Link to="/message">返回消息</Link></div></main>
  if (['trade_group'].includes(conversation.kind)) return <TradeChatPage conversation={conversation} />
  if (['support'].includes(conversation.kind)) return <SupportChatPage conversation={conversation} messages={messages} />
  ...
  return <main className="group-chat-page">
    <header className="chat-header"><StatusBar className="chat-status" /><PageHeader className="chat-page-header" bordered={false} title={conversation.kind === 'trade_group' ? `交易群 · ${conversation.title}` : '王者荣耀 · 专属客服'} left={<button type="button" onClick={back} aria-label="返回消息"><ArrowLeft size={21} /></button>} right={conversation.kind === 'support' ? <button type="button" className="chat-more-button" onClick={() => setSettingsOpen(true)} aria-label="客服消息设置"><MoreHorizontal size={21} /></button> : conversation.orderId ? <Link to={`/orders/${conversation.orderId}`}>订单</Link> : undefined}>{conversation.kind === 'trade_group' ? '买家 · 卖家 · 平台客服萌萌' : '在线 · 服务时间 09:00–24:00'}</PageHeader></header>
    ...
    <div className="chat-log" ref={logRef} role="log" aria-live="polite" aria-label="聊天记录">
      ...
      {hasStructuredBindingCards && <section className="chat-material-card" aria-labelledby="material-card-title"><Heading id="material-card-title" as="h2" variant="subsection">资料核对卡 · 卖家需提供</Heading><div><span>账号 + 密码</span><b>已提供 ✓</b></div><div><span>换绑手机号</span><b>已提供 ✓</b></div></section>}
    ...
    {confirmAction && <div className="chat-confirm-layer"><button type="button" className="chat-confirm-mask" aria-label="取消操作" onClick={closeConfirm} /><section ref={dialogRef} role="alertdialog" aria-modal="true" aria-labelledby="chat-confirm-title"><Heading id="chat-confirm-title" as="h2" variant="dialog">{confirmAction === 'binding' ? '确认换绑' : '反馈验号不符'}</Heading><p>{confirmAction === 'binding' ? '请确认账号资料、手机号和登录状态均核对无误。确认后交易状态将推进，且不可重复操作。' : '提交后将暂停交易并由平台客服介入，确认继续吗？'}</p><footer><button type="button" onClick={closeConfirm}>取消</button><button type="button" className={confirmAction === 'mismatch' ? 'danger' : 'primary'} onClick={() => void confirmTradeAction()}>{confirmAction === 'binding' ? '确认换绑' : '提交不符'}</button></footer></section></div>}
    {conversation.kind === 'support' && <BottomSheet open={settingsOpen} onClose={() => setSettingsOpen(false)} title="消息设置" className="chat-settings-sheet" actions={<button type="button" className="chat-clear-history" onClick={() => { setMessages([]); setPending([]); setHistoryCleared(true); setSettingsOpen(false); setToast('已清空本地聊天记录') }}>清空聊天记录</button>}><div className="chat-setting-list"><div><span><b>置顶消息</b></span><span><ToggleSwitch checked={pinned} onCheckedChange={setPinned} label="置顶消息" /></span></div><div><span><b>消息免打扰</b></span><span><ToggleSwitch checked={muted} onCheckedChange={setMuted} label="消息免打扰" /></span></div><button type="button" onClick={() => { setSettingsOpen(false); setToast('投诉入口已打开') }}><span><b>投诉服务</b></span><em aria-hidden="true">›</em></button><button type="button" onClick={() => navigate('/feedback')}><span><b>问题反馈</b></span><em aria-hidden="true">›</em></button></div></BottomSheet>}
