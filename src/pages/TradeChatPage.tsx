...
import { Camera, Check, ChevronLeft, CirclePlus, FileText, Send, ShieldCheck, X } from 'lucide-react'
import { Link, useNavigate, useSearchParams } from 'react-router-dom'
import { assetPath } from '../components/assetPath'
import { TradeHistory } from '../components/TradeHistory'
import { Heading, StatusBar, TextField, TextAreaField, ChoiceChip } from '../components/ui'
import { getOrderWorkflowPhase } from '../components/orderModel'
import { getConversationPhase, getTradePhaseTitle, getTradeStep, isOrderConversationMatch, isTradePhase, isTradePreviewMode, nextTradePhase, tradePhases, validateTradeIssue, type TradeAction, type TradePhase, type TradeRole } from '../components/tradeFlowModel'
...
const phaseStatus: Partial<Record<TradePhase, OrderStatus>> = { inspection: 'verifying', binding: 'binding', release: 'bind_success', completed: 'completed' }
...
export function TradeChatPage({ conversation }: { conversation: Conversation }) {
  ...
  const step = getTradeStep(phase, activeOrder?.pausedPhase ?? conversation.pausedPhase)
  ...
  const canAct = hasActionContext && ((phase === 'materials' && role === 'seller') || (phase === 'inspection' && role === 'buyer') || (phase === 'binding' && role === 'seller') || (phase === 'release' && role === 'buyer'))
  ...
  return <main className={`group-chat-page trade-d3-page phase-${phase}`} data-node-id={phase === 'materials' ? role === 'seller' ? '3993:127' : '3993:578' : phase === 'inspection' ? role === 'seller' ? '3993:242' : '3993:684' : phase === 'binding' ? role === 'seller' ? '3993:366' : '3993:807' : phase === 'release' ? '3993:916' : phase === 'paused' ? '3993:1038' : '3993:475'}>
    <header className="trade-d3-header"><StatusBar className="chat-status" /><div className="trade-d3-topbar"><button type="button" aria-label="返回消息" onClick={() => navigate('/message?tab=groups')}><ChevronLeft size={21} /></button><button type="button" className="trade-d3-title" aria-label={isHistory ? "已关闭交易群信息" : "交易群信息与预览状态"} disabled={isHistory} onClick={() => open('preview')}><Heading as="h1" variant="page">交易群 · {title}</Heading><small>买家 · 卖家 · 平台客服萌萌</small></button><Link to={activeOrder ? `/orders/${activeOrder.id}` : '/orders'}>订单</Link></div></header>
    {isHistory ? <section className="trade-d3-progress" aria-label="会话已关闭，交易已完成"><div><b>会话已关闭 · 交易已完成</b><small>历史记录仅供查看</small></div></section> : <section className="trade-d3-progress" aria-label={`步骤 ${step} / 4，${getTradePhaseTitle(phase, role)}`}><div><b>{phase === 'paused' && <em>已暂停</em>}步骤 {step} / 4 · {getTradePhaseTitle(phase, role)}</b><small>{phase === 'release' ? '剩 61:04:22' : phase === 'completed' ? '09-09 11:38' : `已 ${step === 1 ? '2' : step === 2 ? '7' : '22'} 分钟`}</small></div><ol aria-hidden="true">{[1, 2, 3, 4].map(i => <li key={i} className={i < step || phase === 'completed' ? 'done' : i === step ? 'current' : ''} />)}</ol></section>}
    <div ref={logRef} className="trade-d3-log" role="log" aria-label="交易群内容">
      ...
      {phase === 'materials' ? <><time className="trade-d3-time">09-09 10:02</time><div className="trade-d3-history"><ShieldCheck size={13} />{role === 'seller' ? '买家' : '你'}已付款 <b>¥{amount}</b>，资金进入平台托管</div><div className="trade-d3-product"><img src={assetPath('assets/games/wzry.png')} alt="" /><span><b>{activeOrder?.productTitle ?? '王者荣耀 QQ区 王者50星 108英雄'}</b><small>{activeOrder?.id ?? conversation.orderId}</small></span><strong>¥{amount}</strong></div></> : phase !== 'paused' && <div className="trade-d3-history"><Check size={14} />{phase === 'inspection' ? '资料同步 · 已完成' : phase === 'binding' ? '买家验号完成 · 已进入换绑' : phase === 'release' ? '验号完成 · 换绑完成 · 等待买家确认放款' : phase === 'completed' ? '验号完成 · 换绑完成 · 已确认放款' : '交易已关闭'}</div>}
      ...
      {!['completed', 'closed', 'paused'].includes(phase) && <section className={`trade-d3-card trade-d3-task ${canAct && ['inspection', 'release'].includes(phase) ? 'critical' : ''}`}>
      ...
      {!canAct && !['completed', 'closed', 'paused'].includes(phase) && <div className="trade-d3-reminders"><button type="button" disabled={cooldown} onClick={() => remind(role === 'buyer' ? '卖家' : '买家')}>催{role === 'buyer' ? '卖家' : '买家'}</button><button type="button" disabled={cooldown} onClick={() => remind('客服')}>催客服</button></div>}
    ...
    <footer className="trade-d3-composer"><input value={text} maxLength={1000} disabled={terminal} onChange={e => setText(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.nativeEvent.isComposing) void send() }} placeholder={terminal ? '会话已关闭' : '发消息…'} aria-label="消息内容" /><button type="button" disabled={terminal} aria-label="选择消息图片" onClick={() => imageRef.current?.click()}><Camera size={20} /></button><button type="button" disabled={terminal || busy} aria-label={text.trim() ? '发送消息' : '更多交易操作'} onClick={() => text.trim() ? void send() : open('preview')}>{text.trim() ? <Send size={20} /> : <CirclePlus size={21} />}</button><input type="file" ref={imageRef} accept="image/jpeg,image/png,image/webp" hidden onChange={e => { const file = e.target.files?.[0]; if (!file) return; if (file.size > 10 * 1024 * 1024) { setToast('图片不能超过 10MB'); return } setImages(previous => [...previous, URL.createObjectURL(file)]) }} /></footer>
