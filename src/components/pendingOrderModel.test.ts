...
describe('shared pending order summary', () => {
  ...
  it('sums the buyer, seller and actionable aftersales entry badges', () => {
    expect(getPendingOrderSummary(createOrderSeed(now), afterSalesFixtures, now)).toEqual({ buyerPendingCount: 2, sellerPendingCount: 2, afterSalePendingCount: 1, totalPendingCount: 5 })
  })
  ...
  it('excludes expired payments and paused trades without needing the profile page to mount', () => {
    ...
    expect(getPendingOrderSummary(orders, [], now + 31 * 60_000)).toEqual({ buyerPendingCount: 1, sellerPendingCount: 0, afterSalePendingCount: 0, totalPendingCount: 1 })
  })

  it('does not count duplicate records, finished orders or platform-side processing', () => {
    const orders = createOrderSeed(now)
    expect(getPendingOrderSummary([...orders, ...orders], [...afterSalesFixtures, ...afterSalesFixtures], now).totalPendingCount).toBe(5)
