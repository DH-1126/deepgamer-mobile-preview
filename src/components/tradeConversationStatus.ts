...
export type TradeConversationStatus = 'pending' | 'materials' | 'inspection' | 'binding' | 'release' | 'completed' | 'closed'
...
const orderStatuses: Record<OrderStatus, TradeConversationStatus> = {
  pending: 'pending', paid: 'materials', verifying: 'inspection', binding: 'binding', bind_success: 'release',
...
export function getTradeConversationStatus(conversation: Conversation, order?: OrderRecord) {
  ...
  if (conversation.historyPreview && conversation.closed) status = 'closed'
    ...
    if (phase === 'completed') status = 'completed'
      ...
      status = previous && ['materials', 'inspection', 'binding', 'release'].includes(previous)
        ? previous as 'materials' | 'inspection' | 'binding' | 'release'
        : 'inspection'
    } else status = phase
  }
